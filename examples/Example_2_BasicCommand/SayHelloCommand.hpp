...
struct HelloParameters : public CommandlineTool::CommandParameters
{
	HelloParameters(std::string name) : Name(name) {}
	virtual ~HelloParameters() {}
...
public:
	...
	*/
	virtual CommandlineTool::CommandArguments CreateArguments(std::map<std::string, std::string> arguments) override
	{
		...
		std::string name;
		if (ContainsArgument(arguments, argName))
		{
			name = arguments[argName];
		}

		return {"say_hello", std::make_shared<HelloParameters>(name)};
	}
